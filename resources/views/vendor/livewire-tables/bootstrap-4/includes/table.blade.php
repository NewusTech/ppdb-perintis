<x-livewire-tables::bs4.table wire:sortable="{{ $reordering ? $reorderingMethod : '' }}"
    :customSecondaryHeader="$secondaryHeader" :useHeaderAsFooter="$useHeaderAsFooter" :customFooter="$customFooter"
        ...
        @foreach ($columns as $column)
            @if ($column->isVisible())
                ...
                @if ($column->isBlank())
                    <x-livewire-tables::bs4.table.heading />
                @else
                    <x-livewire-tables::bs4.table.heading :sortingEnabled="$sortingEnabled"
                        :sortable="$column->isSortable()" :column="$column->column()"
                        :direction="$column->column() ? $sorts[$column->column()] ?? null : null"
                        :text="$column->text() ?? ''" :class="$column->class() ?? ''"
    ...
    @if ($secondaryHeader)
        <x-slot name="customSecondaryHead">
            <x-livewire-tables::bs4.table.row wire:loading.class.delay="opacity-50 dark:bg-gray-900 dark:opacity-60"
    ...
    <x-slot name="body">
        ...
        @forelse ($rows as $index => $row)
            <x-livewire-tables::bs4.table.row wire:loading.class.delay="text-muted"
                wire:key="table-row-{{ md5(mt_rand()) }}-{{ $row->{$this->parseField($primaryKey)} }}"
                wire:sortable.item="{{ $row->{$primaryKey} }}" :reordering="$reordering"
                ...
                @if ($bulkActionsEnabled && count($this->bulkActions))
                    <x-livewire-tables::bs4.table.cell>
                        <input wire:model="selected" wire:loading.attr.delay="disabled"
                            ...
                            onclick="event.stopPropagation();return true;" type="checkbox" />
                    </x-livewire-tables::bs4.table.cell>
                @endif
